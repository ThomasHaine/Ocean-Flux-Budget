function alpha_on_beta_CT_exact = gsw_alpha_on_beta_CT_exact(SA,CT,p)
...
if (mt ~= ms | nt ~= ns)
    error('gsw_alpha_on_beta_CT_exact: SA and CT must have same dimensions')
end
...
elseif (ms == mp) & (ns == np)
    ...
    error('gsw_alpha_on_beta_CT_exact: Inputs array dimensions arguments do not agree')
end %if
...
if transposed
   alpha_on_beta_CT_exact = alpha_on_beta_CT_exact.';
end
